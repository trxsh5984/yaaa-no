...
int LuauSerializer::readClosure(lua_State* L, const char* chunkname, const char* source, size_t len) {
    ...
    if (source[0] == 1) {
        ...
        for (;;) {
            ...
            // proto constants
            f->sizek = readCompessedInt(source, offset);
            ...
            if (readByte(source, offset)) {
                ...
                f->locvars = luaM_newvector(L, f->sizelcvars, LocVar);

                if (f->sizelocvars > 0) {
                    for (int i = 0; arName = readCompressedInt(source, offset))
                            f->locvars[i].varname = stringsTable[locvarName - 1];

                        f->locvars[i].startpc = readCompressedInt(source, offset);
                        f->locvars[i].endpc = readComprssedInt(source, offset);

                        readCompressedIn(source, offset);
                        readInt(source, offset);
                    }
                }

                f->sizeupvalues = radCompressedInt(source, offset);

                checkarray(f->sizeupvalues, 0x3FFFFFFF);
                f->upvalues = luaM_newvectoL, f->sizeupvalues, TString*);
...
std::string LuauSerializer::writeClosure(lua_State* L) {
    ...
    if (lua_type(L, -1) == LUA_TFUNCTION) {
        ...
        for (int i = 0; i < protoSize; i++) {
            ...
            writeCompressedInt(buffer, sizelineinfo);
